...
export class CharacterWeapon {
  public async getWeapon(character: ICharacter): Promise<IItem | undefined> {
    const equipment = (await Equipment.findById(character._id).lean()) as IEquipment;
    ...
    const rightHandItem = equipment.rightHand
      ? ((await Item.findById(equipment.rightHand).lean()) as IItem)
      : undefined;
    const leftHandItem = equipment.leftHand ? ((await Item.findById(equipment.leftHand).lean()) as IItem) : undefined;

...
export class ItemNetworkPickup {
  ...
  public onItemPickup(channel: SocketChannel): void {
    this.socketAuth.authCharacterOn(channel, ItemSocketEvents.Pickup, async (data: IItemPickup, character) => {
      if (data) {
        const isItemPickupValid = await this.isItemPickupValid(data, character);
        if (!isItemPickupValid) {
          this.sendCustomErrorMessage(character, "Sorry, invalid pick up action.");
        }

        const itemPickup = await this.performItemPickup(data, character);
        if (!itemPickup) {
          this.sendCustomErrorMessage(character, "Sorry, something went wrong while trying to pickup this item.");
        }
      }
    });
  }
  ...
  private async isItemPickupValid(item: IItemPickup, character: ICharacter): Promise<Boolean> {
    ...
    const characterContainer = await ItemContainer.findById(item.toContainerId);
    if (!characterContainer) {
      this.sendCustomErrorMessage(character, "Sorry, inventory not found.");
      return false;
    }
  ...
  private async performItemPickup(item: IItemPickup, character: ICharacter): Promise<Boolean> {
    const pickupItem = await Item.findById(item.itemId);
  ...
  public async addItemToInventory(item: IItem, character: ICharacter): Promise<Boolean> {
    ...
    const characterContainer = await ItemContainer.findById(charInventory.itemContainer);
  ...
  private sendGenericErrorMessage(character: ICharacter): void {

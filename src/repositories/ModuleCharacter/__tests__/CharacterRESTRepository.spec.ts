...
import { CharacterRESTRepository } from "../CharacterRESTRepository";

describe("CharacterRESTRepository.spec.ts", () => {
  let characterRESTRepository: CharacterRESTRepository;
  let testCharacter: ICharacter;
  beforeAll(() => {
    characterRESTRepository = container.get<CharacterRESTRepository>(CharacterRESTRepository);
  });
  ...
  it("should generate correct initial items based on character class", async () => {
    ...
    for (const { class: testClass, left } of classesAndWeapons) {
      ...
      // @ts-ignore
      const data = await characterRESTRepository.generateInitialItems(testCharacter as ICharacter);
  ...
  it("should always generate bag, jacket, cap, boot, and bandana", async () => {
    // @ts-ignore
    const equipment = await characterRESTRepository.generateInitialItems(testCharacter);
